...
func (r *MachineReconciler) isDeleteNodeAllowed(ctx context.Context, cluster *clusterv1.Cluster, machine *clusterv1.Machine) error {
	...
	if cluster.Spec.ControlPlaneRef != nil {
		controlPlane, err := external.Get(ctx, r.Client, cluster.Spec.ControlPlaneRef.FullRef(cluster.Namespace))
		if apierrors.IsNotFound(err) {
...
func (r *MachineReconciler) reconcileDeleteExternal(ctx context.Context, m *clusterv1.Machine, ref *clusterv1.LocalObjectReference) (*unstructured.Unstructured, error) {
	...
	obj, err := external.Get(ctx, r.Client, ref.FullRef(m.Namespace))
